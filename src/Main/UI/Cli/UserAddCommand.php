...
use App\Main\Infrastructure\QueryBus\GetUsersByOptionsQuery;
...
class UserAddCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$email || !preg_match("/^[a-zA-Z-' ]*$/", $email)) {
            $io->error('Utworzenie nowego użytkownika zostało anulowane! Błędna nazwa użytkownika.');
            return Command::FAILURE;
        }

        $dto = new UserDto($email);
        $emailCheck = $this->queryBus->handle(new GetUsersByOptionsQuery($dto));

        dump($emailCheck);
        dd('end');

        if ($this->userRepository->findAllUserByAttrAndValue('username', $email)) {
